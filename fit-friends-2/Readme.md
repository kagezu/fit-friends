...
BACKEND - адрес бекенда, по умолчанию используется http://localhost:3333  
...
После создания проекта вам доступны следующие сценарии. Обратите внимание, для запуска сценария, вы должны находится в директории проекта (`./fit-friends-2`).
...
### Сборка проекта и создание образа для докера

```bash
npm run buildImage
```

Запуск сборки приложения и создание образа.

После сборки будет доступен образ с собранным приложением.
